...
class AccountManagerApp:
    def __init__(self, root):
        ...
        # 启动签到检查线程
        self.check_thread = threading.Thread(target=self.check_signin_time_thread, daemon=True)
        self.check_thread.start()

    def setup_ui(self):
      # 左侧账号列表栏
      self.account_frame = tk.LabelFrame(self.root, text="账号列表", padx=5, pady=5)
      self.account_frame.grid(row=0, column=0, rowspan=5, padx=5, pady=5, sticky="ns")
      ...
      self.account_list.bind("<<ListboxSelect>>", self.load_account_config)  # 选中账号加载配置

      # 右侧配置栏
      self.config_frame = tk.LabelFrame(self.root, text="配置参数", padx=5, pady=5)
      ...
      for i, (label_text, entry) in enumerate(zip(labels, entries)):
          tk.Label(self.config_frame, text=label_text).grid(row=i, column=0, padx=5, pady=2, sticky="e")
          entry.grid(row=i, column=1, padx=5, pady=2)

      # 保存配置按钮
      self.save_config_button = tk.Button(self.config_frame, text="保存配置", command=self.save_account_config)
    ...
    def signin(self, username=None):
      ...
      if not selected_account:
          self.root.after(0, lambda: messagebox.showwarning("警告", "请选择一个账号进行签到"))
          return
      ...
      if not os.path.exists(config_file):
          self.root.after(0, lambda: messagebox.showwarning("警告", "该账号缺少配置文件"))
          return
      ...
      # 加载和检查cookies
      login_manager = LoginManager(self.logger, cookie_file=os.path.join(account_path, 'cookies.json'))
      ...
      if login_manager.is_cookies_expired():
          ...
          if login_manager.login(selected_account, password):
              ...
              self.root.after(0, lambda: messagebox.showwarning("警告", "登录失败，请检查账号和密码"))
              return
      ...
      # 使用after在主线程中更新日志
      self.root.after(0, lambda: self.log_message(f"账号 {selected_account} 签到请求已发送，结果: {result}"))
    ...
    def check_signin_time_thread(self):
        ...
        while True:
            for account in os.listdir('./data'):
                ...
                # 当前时间与签到时间对比
                sign_in_time = datetime.strptime(sign_in_time_str, "%H:%M:%S").time()
